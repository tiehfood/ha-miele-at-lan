"""Pure-Python tests for the GLOBAL_USER_REQ payload builder. No HA / network."""
...
from custom_components.miele_lan.api import build_user_request_payload  # noqa: E402
...
def test_light_on_matches_capture() -> None:
    payload = build_user_request_payload(OPCODE_LIGHT_ON)
...
def test_light_off_matches_capture() -> None:
    payload = build_user_request_payload(OPCODE_LIGHT_OFF)
...
def test_switch_on_matches_capture() -> None:
    payload = build_user_request_payload(OPCODE_SWITCH_ON)
...
def test_switch_off_matches_capture() -> None:
    payload = build_user_request_payload(OPCODE_SWITCH_OFF)
...
def test_payload_is_aes_block_aligned() -> None:
    for op in (OPCODE_LIGHT_ON, OPCODE_LIGHT_OFF, OPCODE_SWITCH_ON, OPCODE_SWITCH_OFF):
        assert len(build_user_request_payload(op)) % 16 == 0
...
def test_rejects_out_of_range_opcode() -> None:
    ...
    with pytest.raises(ValueError):
        build_user_request_payload(-1)
    with pytest.raises(ValueError):
        build_user_request_payload(256)
